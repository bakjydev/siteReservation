...
const RESOURCE_REFINERS = {
    ...
    title: String,
    businessHours: internal.identity,
    extendedProps: internal.identity,
    // event-ui
    eventEditable: Boolean,
    eventStartEditable: Boolean,
    eventDurationEditable: Boolean,
    eventConstraint: internal.identity,
    eventOverlap: Boolean,
    eventAllow: internal.identity,
    eventClassNames: internal.parseClassNames,
...
function parseResource(raw, parentId = '', store, context) {
    let { refined, extra } = internal.refineProps(raw, RESOURCE_REFINERS);
    let resource = {
        id: refined.id || (PRIVATE_ID_PREFIX + internal.guid()),
        parentId: refined.parentId || parentId,
        title: refined.title || '',
        businessHours: refined.businessHours ? internal.parseBusinessHours(refined.businessHours, context) : null,
        ui: internal.createEventUi({
    ...
    getEvents() {
        ...
        for (let instanceId in instances) {
            ...
            if (def.resourceIds.indexOf(thisResourceId) !== -1) { // inefficient!!!
                eventApis.push(new internal.EventImpl(context, def, instance));
            }
        }
        return eventApis;
    }
...
*/
class ResourceSplitter extends internal.Splitter {
    getKeyInfo(props) {
...
const DEFAULT_RESOURCE_ORDER = internal.parseFieldSpecs('id,title');
...
class ResourceLabelContainer extends internal.BaseComponent {
    constructor() {
        super(...arguments);
    ...
    render() {
        ...
        return (preact.createElement(internal.ViewContextType.Consumer, null, (context) => {
            ...
            return (preact.createElement(internal.ContentContainer, Object.assign({}, props, { elAttrs: Object.assign(Object.assign({}, props.elAttrs), { 'data-resource-id': props.resource.id, 'data-date': props.date ? internal.formatDayString(props.date) : undefined }), renderProps: renderProps, generatorName: "resourceLabelContent", generator: options.resourceLabelContent || renderInnerContent, classNameGenerator: options.resourceLabelClassNames, didMount: options.resourceLabelDidMount, willUnmount: options.resourceLabelWillUnmount })));
        }));
    }
}
...
class ResourceCell extends internal.BaseComponent {
    render() {
        let { props } = this;
...
class ResourceDayHeader extends internal.BaseComponent {
    constructor() {
        super(...arguments);
        this.buildDateFormat = internal.memoize(buildDateFormat);
    }
    render() {
    ...
    renderDateCell(date, dateFormat, todayRange, colSpan, resource, isSticky) {
        ...
        return props.datesRepDistinctDays ? (preact.createElement(internal.TableDateCell, { key: date.toISOString() + keyPostfix, date: date, dateProfile: props.dateProfile, todayRange: todayRange, colCnt: props.dates.length * props.resources.length, dayHeaderFormat: dateFormat, colSpan: colSpan, isSticky: isSticky, extraRenderProps: extraRenderProps, extraDataAttrs: extraDataAttrs })) : (preact.createElement(internal.TableDowCell // we can't leverage the pure-componentness becausae the extra* props are new every time :(
...
function buildDateFormat(dayHeaderFormat, datesRepDistinctDays, dayCnt) {
    return dayHeaderFormat || internal.computeFallbackHeaderFormat(datesRepDistinctDays, dayCnt);
}
...
class VResourceJoiner {
    constructor() {
        this.joinDateSelection = internal.memoize(this.joinSegs);
        this.joinBusinessHours = internal.memoize(this.joinSegs);
        this.joinFgEvents = internal.memoize(this.joinSegs);
        this.joinBgEvents = internal.memoize(this.joinSegs);
        this.joinEventDrags = internal.memoize(this.joinInteractions);
        this.joinEventResizes = internal.memoize(this.joinInteractions);
    }
    /*
...
*/
class VResourceSplitter extends internal.Splitter {
    getKeyInfo(props) {
        let { resourceDayTableModel } = props;
        let hash = internal.mapHash(resourceDayTableModel.resourceIndex.indicesById, (i) => resourceDayTableModel.resources[i]); // :(
        hash[''] = {};
        return hash;
    }
    getKeysForDateSpan(dateSpan) {
...
function ensureGroupNodes(resourceNode, nodes, groupSpec) {
    ...
    if (groupSpec.order) {
        for (newGroupIndex = 0; newGroupIndex < nodes.length; newGroupIndex += 1) {
            ...
            if (node.group) {
                let cmp = internal.flexibleCompare(groupValue, node.group.value) * groupSpec.order;
                if (cmp === 0) {
                    groupNode = node;
                    break;
                }
...
function insertResourceNodeInSiblings(resourceNode, siblings, orderSpecs) {
    ...
    for (i = 0; i < siblings.length; i += 1) {
        let cmp = internal.compareByFieldSpecs(siblings[i].resourceFields, resourceNode.resourceFields, orderSpecs); // TODO: pass in ResourceApi?
