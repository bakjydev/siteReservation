...
class ResourceDataAdder {
    constructor() {
        this.filterResources = internal.memoize(filterResources);
    }
...
function filterResources(resourceStore, doFilterResourcesWithEvents, eventStore, activeRange) {
    if (doFilterResourcesWithEvents) {
        ...
        return internal.filterHash(resourceStore, (resource, resourceId) => hasEvents[resourceId]);
    }
    return resourceStore;
}
function filterEventInstancesInRange(eventInstances, activeRange) {
    return internal.filterHash(eventInstances, (eventInstance) => internal.rangesIntersect(eventInstance.range, activeRange));
}
...
class ResourceEventConfigAdder {
    constructor() {
        this.buildResourceEventUis = internal.memoize(buildResourceEventUis, internal.isPropsEqual);
        this.injectResourceEventUis = internal.memoize(injectResourceEventUis);
    }
    transform(viewProps, calendarProps) {
        if (!calendarProps.viewSpec.optionDefaults.needsResourceData) {
            return {
...
function buildResourceEventUis(resourceStore) {
    return internal.mapHash(resourceStore, (resource) => resource.ui);
}
function injectResourceEventUis(eventUiBases, eventDefs, resourceEventUis) {
    return internal.mapHash(eventUiBases, (eventUi, defId) => {
...
function injectResourceEventUi(origEventUi, eventDef, resourceEventUis) {
    ...
    return internal.combineEventUis(parts);
}
...
function parseResourceSource(input) {
    ...
    if (inputObj) {
        let { refined, extra } = internal.refineProps(inputObj, RESOURCE_SOURCE_REFINERS);
        warnUnknownProps(extra);
        let metaRes = buildResourceSourceMeta(refined);
        if (metaRes) {
            return {
                _raw: input,
                sourceId: internal.guid(),
...
function handleRangeChange(source, activeRange, refetchResourcesOnNavigate, context) {
    if (refetchResourcesOnNavigate &&
        ...
        (!source.fetchRange || !internal.rangesEqual(source.fetchRange, activeRange))) {
...
internal.CalendarImpl.prototype.addResource = function (input, scrollTo = true) {
...
internal.CalendarImpl.prototype.getResourceById = function (id) {
    id = String(id);
...
internal.CalendarImpl.prototype.getResources = function () {
    let currentState = this.getCurrentData();
...
internal.CalendarImpl.prototype.getTopLevelResources = function () {
    let currentState = this.getCurrentData();
    let { resourceStore } = currentState;
    let resourceApis = [];
    if (resourceStore) { // guard against calendar with no resource functionality
...
internal.CalendarImpl.prototype.refetchResources = function () {
    this.dispatch({
...
function isPropsValidWithResources(combinedProps, context) {
    ...
    for (let resourceId in sets) {
        ...
        if (resourceId && sets['']) { // current segment is not the non-resource one, and there IS a non-resource one
            props = Object.assign(Object.assign({}, props), { eventStore: internal.mergeEventStores(sets[''].eventStore, props.eventStore), eventUiBases: Object.assign(Object.assign({}, sets[''].eventUiBases), props.eventUiBases) });
        }
        if (!internal.isPropsValid(props, context, { resourceId }, filterConfig.bind(null, resourceId))) {
            return false;
        }
    }
    return true;
...
const OPTION_REFINERS = {
    initialResources: internal.identity,
    resources: internal.identity,
    eventResourceEditable: Boolean,
    refetchResourcesOnNavigate: Boolean,
    resourceOrder: internal.parseFieldSpecs,
    filterResourcesWithEvents: Boolean,
    resourceGroupField: String,
    resourceAreaWidth: internal.identity,
    resourceAreaColumns: internal.identity,
    resourcesInitiallyExpanded: Boolean,
    datesAboveResources: Boolean,
    needsResourceData: Boolean,
    resourceAreaHeaderClassNames: internal.identity,
    resourceAreaHeaderContent: internal.identity,
    resourceAreaHeaderDidMount: internal.identity,
    resourceAreaHeaderWillUnmount: internal.identity,
    resourceGroupLabelClassNames: internal.identity,
    resourceGroupLabelContent: internal.identity,
    resourceGroupLabelDidMount: internal.identity,
    resourceGroupLabelWillUnmount: internal.identity,
    resourceLabelClassNames: internal.identity,
    resourceLabelContent: internal.identity,
    resourceLabelDidMount: internal.identity,
    resourceLabelWillUnmount: internal.identity,
    resourceLaneClassNames: internal.identity,
    resourceLaneContent: internal.identity,
    resourceLaneDidMount: internal.identity,
    resourceLaneWillUnmount: internal.identity,
    resourceGroupLaneClassNames: internal.identity,
    resourceGroupLaneContent: internal.identity,
    resourceGroupLaneDidMount: internal.identity,
    resourceGroupLaneWillUnmount: internal.identity,
};
const LISTENER_REFINERS = {
    resourcesSet: internal.identity,
    resourceAdd: internal.identity,
    resourceChange: internal.identity,
    resourceRemove: internal.identity,
};

internal.EventImpl.prototype.getResources = function () {
    let { calendarApi } = this._context;
    return this._def.resourceIds.map((resourceId) => calendarApi.getResourceById(resourceId));
};
internal.EventImpl.prototype.setResources = function (resources) {
    let resourceIds = [];
...
registerResourceSourceDef({
    ...
    fetch(arg, successCallback, errorCallback) {
        ...
        internal.unpromisify(func.bind(null, publicArg), (rawResources) => successCallback({ rawResources }), errorCallback);
    },
});
...
registerResourceSourceDef({
    ...
    fetch(arg, successCallback, errorCallback) {
        ...
        internal.requestJson(meta.method, meta.url, requestParams).then(([rawResources, response]) => {
